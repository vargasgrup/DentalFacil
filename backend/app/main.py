...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        raise
    try:
        from app.ensure_patient_document_unique import ensure_patient_document_unique

        ensure_patient_document_unique()
    except Exception as exc:  # noqa: BLE001
        logger.error("ensure_patient_document_unique FAILED: %s", exc, exc_info=True)
    try:
        from app.ensure_patient_lifecycle_schema import ensure_patient_lifecycle_schema

        ensure_patient_lifecycle_schema()
    except Exception as exc:  # noqa: BLE001
        logger.error("ensure_patient_lifecycle_schema FAILED: %s", exc, exc_info=True)
        raise
    try:
        from app.ensure_patient_especialidades_schema import (
            ensure_patient_especialidades_schema,
        )

        ensure_patient_especialidades_schema()
    except Exception as exc:  # noqa: BLE001
        logger.error(
            "ensure_patient_especialidades_schema FAILED: %s", exc, exc_info=True
        )
        raise
    try:
...
# Desktop installs set CORS_ORIGINS=*. Starlette rejects allow_origins=["*"] with
# allow_credentials=True, so reflect any origin via regex (LAN + localhost variants).
