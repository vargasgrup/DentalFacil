...
class ClinicSettings(Base):
    ...
    # Horario
    hora_apertura: Mapped[str] = mapped_column(String(5), default="08:00")  # HH:MM
    hora_cierre: Mapped[str] = mapped_column(String(5), default="20:00")

    # Datos del centro (Perú) — vacíos → fallback a CLINIC_* env
    ...
    # Ruta relativa bajo app/assets/ (ej. uploads/clinic-logo.png)
    logo_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    # Catálogo editable del centro (JSON array de strings). None → defaults.
    especialidades: Mapped[list | None] = mapped_column(JSON, nullable=True)
    # Recordatorios WhatsApp (persistidos; antes solo en memoria)
