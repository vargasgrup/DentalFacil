...
function NavLink({
  ...
  return (
    <Link
      ...
      title={label}
      className={[
        "group relative flex items-center rounded-xl text-sm font-semibold tracking-tight transition-smooth",
        collapsed
          ? "min-h-[2.75rem] justify-center px-1.5 py-2"
          : "min-h-[2.75rem] gap-3 px-2.5 py-2",
        active
...
export function Sidebar({
  ...
  const { user } = useAuth();
  const principalVisible = NAV_PRINCIPAL.filter((item) =>
    canAccessHref(user, item.href)
  );
  const sistemaVisible = NAV_SISTEMA.filter((item) =>
    canAccessHref(user, item.href)
  );
  ...
  return (
    ...
    >
      <NavSection
        title="Principal"
        items={principalVisible}
        onNavigate={onNavigate}
        collapsed={collapsed}
      />
      <NavSection
        title="Sistema"
        items={sistemaVisible}
        onNavigate={onNavigate}
        collapsed={collapsed}
      />
    </nav>
